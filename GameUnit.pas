...
Begin
    ...
    Begin
        ...
        Cells[I][J].SetImage(Nil); //Убираем изображение ячейки
        Cells[I][J].SetQueenParam(False); //Убираем параметр Queen ячейки
...
Begin
    ...
    Begin
        Cells[Ti][Tj].SetBackColor(YellowColor); //Устанавливаем желтый цвет фона для ячейки
        Cells[Ti][Tj].Activate(); //Активируем ячейку
        Setlength(Simplesteps, Length(Simplesteps) + 1); //Увеличиваем длину массива Simplesteps на 1
        Simplesteps[High(Simplesteps)] := Cells[Ti, Tj]; //Добавляем текущую ячейку в массив Simplesteps
